...
export interface CoffeeStainProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /** Number of random stains */
  count?: number;
  /** Custom stain configurations */
  stains?: StainConfig[];
  /** Show aged paper effect */
  agedPaper?: boolean;
  /** Show paper texture */
  paperTexture?: boolean;
  /** Show burn/dark edges */
  burnEdges?: boolean;
  /** Show edge darkening */
...
export const CoffeeStain = forwardRef<HTMLDivElement, CoffeeStainProps>(
  (
    ...
    const stains = useMemo(() => 
      customStains || generateRandomStains(count),
      [customStains, count]
    );
    ...
    return (
      ...
      >
        {stains.map((stain, i) => {
          const styleClass = stain.type === 'ring' 
            ? `${styles.ring} ${stain.size === 'lg' ? styles.ring1 : styles.ring2}`
            : `${styles.stain} ${styles[stainStyles[i % stainStyles.length]]}`;
